...
# =====================================
# SECTION 4: Build the Conditional PixelCNN Model
# =====================================

def build_conditional_pixelcnn(img_shape, num_classes, num_filters=64, num_layers=7):
    """
    Build a Conditional PixelCNN using masked convolutions and label conditioning.

    Args:
        img_shape: Shape of input images (H, W, C).
        num_classes: Number of class labels for conditioning.
        num_filters: Number of filters in masked conv layers.
        num_layers: Number of masked 'B' convolution layers.
    ...
    # Expand label to match image spatial dimensions
    label_map = layers.Dense(np.prod(img_shape), activation="relu")(label_input)
    label_map = layers.Reshape(img_shape)(label_map)

    # Concatenate label map with image input
    x = layers.Concatenate(axis=-1)([input_image, label_map])

    # First layer: Masked Conv A
    x = MaskedConv2D(filters=num_filters, kernel_size=(7, 7), mask_type='A', name="masked_conv_A")(x)
    x = layers.ReLU()(x)

    # Stack Masked Conv B layers
    for i in range(num_layers):
        x = MaskedConv2D(filters=num_filters, kernel_size=(3, 3), mask_type='B', name=f"masked_conv_B_{i}")(x)
        x = layers.ReLU()(x)

    # Final output layer to predict pixel value (sigmoid for [0, 1])
...
if __name__ == "__main__":
    ...
    # Train real-only classifier
    classifier_real = train_real_only_classifier_ar(
    ...
    # Train real + synthetic data classifier
    classifier_combined = train_classifier_on_real_plus_synthetic_ar(
