...
import yaml
from tensorflow.keras.layers import LayerNormalization, MultiHeadAttention, Dense, Add, Reshape, Layer
...
class MaskedConv2D(layers.Layer):
    ...
    - Mask type 'A' is used for the first layer (blocks current pixel).
    - Mask type 'B' is used for all subsequent layers (allows current pixel).
    """
    def __init__(self, filters, kernel_size, mask_type, **kwargs):
        ...
        # Underlying conv layer with no bias (mask will be applied manually)
        self.conv = layers.Conv2D(filters, kernel_size, padding='same', use_bias=False)

    def build(self, input_shape):
        # Build the internal Conv2D layer with input shape
        ...
        # Create an all-ones mask of shape matching the convolutional kernel
        mask = np.ones((kh, kw, in_channels, out_channels), dtype=np.float32)

        center_h = kh // 2
        center_w = kw // 2

        # Mask the future pixels:
        # - For both 'A' and 'B', mask out everything *below* the center row
        # - For 'A', also mask out the *center pixel*
        # - For 'B', allow center pixel (but not the right ones)
        mask[center_h, center_w + (self.mask_type == 'B'):, :, :] = 0
        mask[center_h + 1:, :, :, :] = 0

        # Convert the static numpy mask to a TensorFlow constant
        self.mask = tf.constant(mask, dtype=tf.float32)

    def call(self, inputs):
        # Apply the mask to the convolution kernel weights
        masked_kernel = self.conv.kernel * self.mask

        # Perform the masked convolution manually using tf.nn.conv2d
...
# =====================================
# SECTION 4A: Positional Embedding Layer
# =====================================

class PositionalEmbedding2D(tf.keras.layers.Layer):
    """
    Learnable 2D positional embeddings to inject spatial information into feature maps.
    Combines separate embeddings for height and width axes.
    """
    def __init__(self, height, width, dim):
        ...
        # Learnable embeddings for each row (height) and column (width)
    ...
    def call(self, inputs):
        # Expand and broadcast row and column embeddings to create 2D positional map
        h_embed = tf.expand_dims(self.row_embedding, axis=1)  # (H, 1, D)
        w_embed = tf.expand_dims(self.col_embedding, axis=0)  # (1, W, D)
        pos = h_embed + w_embed  # (H, W, D) via broadcasting

        # Add batch dimension: (1, H, W, D)
        pos = tf.expand_dims(pos, axis=0)

        # Add positional encoding to input feature map: (B, H, W, D)
        return inputs + pos


# =====================================
# SECTION 4B: Transformer Block
# =====================================
...
class TransformerBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, embed_dim, num_heads, ff_dim):
        ...
        self.ffn = tf.keras.Sequential([
            tf.keras.layers.Dense(ff_dim, activation="relu"),  # Expand
    ...
    def call(self, inputs):
        # First residual block: Self-attention + skip connection + normalization
        attn_output = self.att(inputs, inputs)  # Self-attention
        x = self.norm1(inputs + attn_output)

        # Second residual block: Feed-forward + skip connection + normalization
        ffn_output = self.ffn(x)
        return self.norm2(x + ffn_output)


# =====================================
# SECTION 4C: Build the Conditional PixelCNN Model (with Transformer)
# =====================================

def build_conditional_pixelcnn(img_shape, num_classes, num_filters=64, num_layers=4):
    """
    Build a Conditional PixelCNN using masked convolutions + transformer attention.

    Args:
        img_shape: Shape of input images (H, W, C).
        num_classes: Number of class labels.
        num_filters: Filters in masked conv layers.
        num_layers: Number of masked 'B' conv layers before attention.
    ...
    # Embed class label into image shape
    label_map = layers.Dense(np.prod(img_shape), activation="relu")(label_input)
    label_map = layers.Reshape(img_shape)(label_map)

    x = layers.Concatenate(axis=-1)([input_image, label_map])  # (B, H, W, C + label_dim)

    # First MaskedConv2D (A)
    x = MaskedConv2D(filters=num_filters, kernel_size=(7, 7), mask_type='A')(x)
    x = layers.ReLU()(x)

    # MaskedConv2D (B) stack
    for i in range(num_layers):
        x = MaskedConv2D(filters=num_filters, kernel_size=(3, 3), mask_type='B', name=f"masked_conv_B_{i}")(x)
        x = layers.ReLU()(x)

    # Positional Embedding
    h, w, c = x.shape[1:]
    x = PositionalEmbedding2D(height=h, width=w, dim=c)(x)

    # Flatten spatial dims: (B, H*W, C)
    x = layers.Reshape((h * w, c))(x)

    # Transformer Block
    x = TransformerBlock(embed_dim=c, num_heads=4, ff_dim=2 * c)(x)

    # Reshape back to image: (B, H, W, C)
    x = layers.Reshape((h, w, c))(x)

    # Final 1x1 conv layer for pixel prediction
    output = layers.Conv2D(1, kernel_size=1, activation="sigmoid", name="output")(x)

    return models.Model(inputs=[input_image, label_input], outputs=output, name="Conditional_PixelCNN_Transformer")


# =====================================
# SECTION 5A: Define Loss Function and Optimizer
# =====================================
...
# =====================================
# SECTION 5B: Training Step and Sampling Utilities
# =====================================
...
# =====================================
# SECTION 5C: Full Training Loop (with Logging and Early Stopping)
# =====================================
...
# =====================================
# SECTION 15: Log Comparison Results
# =====================================
