...
# === WGAN-GP SPECIFIC ===
EXPERIMENT_NAME = "conditional_wgan_gp"
LOG_FILE = f"{EXPERIMENT_NAME}_result.txt"
CHECKPOINT_DIR = f"{EXPERIMENT_NAME}_checkpoints"
WGAN_SYNTHETIC_DIR = f"{EXPERIMENT_NAME}_synthetic_samples"
...
os.makedirs(WGAN_SYNTHETIC_DIR, exist_ok=True)
...
""" EXPERIMENT 1 """
# Ensure both FIXED_NOISE & FIXED_LABELS_ONEHOT have exactly the same size
N_SAMPLES = (NUM_CLASSES * (100 // NUM_CLASSES))  # FIXED_NOISE.shape[0] == FIXED_LABELS_ONEHOT.shape[0]
""" EXPERIMENT 1 """
# Fixed noise and labels for consistent FID evaluation
FIXED_NOISE = np.random.normal(0, 1, (N_SAMPLES, LATENT_DIM))
FIXED_LABELS = np.tile(np.arange(NUM_CLASSES), N_SAMPLES // NUM_CLASSES).reshape(-1, 1)
...
train_data = (train_data - 0.5) * 2.0  # Scale to [-1, 1]
...
def calculate_fid(real_images, generated_images):
    ...
    # Calculate FID
    ssdiff = tf.reduce_sum((mu_real - mu_fake) ** 2)
    covmean = sqrtm(sigma_real @ sigma_fake)
    if np.iscomplexobj(covmean):
        covmean = covmean.real
    if np.isnan(covmean).any():
        return 1e6  # Penalize invalid scores
...
# -------------------------------------
# SECTION 4: Generator Model (WGAN-GP)
# -------------------------------------
def build_generator():
    noise_input = layers.Input(shape=(LATENT_DIM,))
    label_input = layers.Input(shape=(NUM_CLASSES,))

    # Dense projection of noise
    ...
    x = layers.Concatenate(axis=-1)([x, label_embedding])  # Shape: (5, 5, 257)
    ...
    return models.Model([noise_input, label_input], out, name="WGAN_Generator")


# -------------------------------------
# SECTION 5: Critic Model (WGAN-GP)
# -------------------------------------
def build_critic_discriminator():
    ...
    # Concatenate image and label in the channel dimension
    merged = layers.Concatenate(axis=-1)([img_input, label_map])  # Shape: (40, 40, 2)

    # Initialize x from merged
    x = merged

    """ EXPERIMENT 2: Using SpectralNormalization(layers.Conv2D) instead of just layers.Conv2D() """
    # Conv Blocks
    ...
    # Extra layer
    x = SpectralNormalization(layers.Conv2D(512, kernel_size=3, strides=1, padding='same'))(x)
    x = layers.LayerNormalization()(x)
    x = layers.LeakyReLU(0.2)(x)

    x = layers.Flatten()(x)
    out = layers.Dense(1)(x)  # No activation: outputs raw Wasserstein score

    return models.Model([img_input, label_input], out, name="WGAN_Critic")


# -------------------------------------
# SECTION 6: Gradient Penalty
# -------------------------------------
def gradient_penalty(critic, real_images, fake_images, labels):
    """Enforces Lipschitz constraint by penalizing the gradient norm"""
    ...
    with tf.GradientTape() as gp_tape:
        ...
        # Get the critic's output for interpolated images
        validity_interpolated = critic([interpolated, labels], training=True)

    # Compute gradients of critic scores w.r.t. interpolated images
    gradients = gp_tape.gradient(validity_interpolated, interpolated)

    # Calculate gradient norm
    grad_norm = tf.sqrt(tf.reduce_sum(tf.square(gradients), axis=[1, 2, 3]) + 1e-12)
...
# -------------------------------------
# SECTION 7: Generate Samples per Class
# -------------------------------------
def generate_samples_per_class(generator, samples_per_class=1000, output_dir=WGAN_SYNTHETIC_DIR):
...
# -------------------------------------
# SECTION 8: CNN Classifier for Evaluation
# -------------------------------------
def build_classifier(input_shape=IMG_SHAPE, num_classes=NUM_CLASSES):
    model = models.Sequential(name="WGAN_Evaluator")
...
# --------------------------------------------------
# SECTION 9: Train & Evaluate Real-Only Baseline Classifier
# --------------------------------------------------
def train_classifier_on_real_only(test_data, test_labels):
    ...
    _, acc = classifier.evaluate(test_data, test_labels, verbose=0)
    log_message(f"[BASELINE] Classifier accuracy (Real only): {acc:.4f}")

    return acc


# -------------------------------------
# SECTION 10: Load WGAN-GP Synthetic Samples
# -------------------------------------
def load_synthetic_samples(directory=WGAN_SYNTHETIC_DIR):
    gen_data = []
    gen_labels = []
    ...
    log_message(f"Loaded synthetic WGAN-GP data: X_gen.shape={X_gen.shape}, y_gen.shape={y_gen.shape}")
...
# -------------------------------------
# SECTION 11: Save Generator Outputs for Visual Preview
# -------------------------------------
def save_samples(generator, epoch, n=9):
    ...
    output_path = os.path.join(CHECKPOINT_DIR, f"wgan_gp_generated_epoch_{epoch}.png")
...
# -----------------------------------------
# SECTION 12: Preview Synthetic Samples from Disk
# -----------------------------------------
def preview_synthetic_samples(directory=WGAN_SYNTHETIC_DIR):
    ...
    plt.tight_layout()
    preview_path = os.path.join(CHECKPOINT_DIR, "synthetic_preview_wgan_gp.png")
...
# -----------------------------------------
# SECTION 13: WGAN-GP Training Loop
# -----------------------------------------
def train_wgan_gp():
    log_message("Initializing Conditional Generator & Critic...")
    generator = build_generator()
    critic = build_critic_discriminator()

    g_optimizer = Adam(LR, BETA_1)
    c_optimizer = Adam(LR, BETA_1)

    best_fid = float("inf")

    patience = config["early_stopping"]["patience"]
    monitor_metric = config["early_stopping"]["monitor_metric"]
    early_stop_enabled = config["early_stopping"]["enabled"]

    wait = 0  # Counter for patience

    for epoch in trange(EPOCHS, desc="Training WGAN-GP"):
        ...
        for _ in range(5):  # Train critic more than generator
            ...
            # === EXPERIMENT 3: Add Label Noise (10% chance) ===
            if np.random.rand() < 0.1:
                real_imgs, fake_imgs = fake_imgs, real_imgs
                real_lbls, fake_labels_onehot = fake_labels_onehot, real_lbls

            with tf.GradientTape() as tape:
                real_validity = critic([real_imgs, real_lbls], training=True)
                fake_validity = critic([fake_imgs, fake_labels_onehot], training=True)
                gp = gradient_penalty(critic, real_imgs, fake_imgs, fake_labels_onehot)
                c_loss = tf.reduce_mean(fake_validity) - tf.reduce_mean(real_validity) + 15.0 * gp # EXPERIMENT 2: λ = 15
        ...
        gen_labels_onehot = tf.keras.utils.to_categorical(gen_labels, NUM_CLASSES)

        with tf.GradientTape() as tape:
            generated = generator([noise, gen_labels_onehot], training=True)
            fake_validity = critic([generated, gen_labels_onehot], training=True)
        ...
        # Every 100 epochs: evaluate and save checkpoints
        if (epoch + 1) % 100 == 0:
            log_message(f"Epoch {epoch + 1}/{EPOCHS} starting...")

            # fid = calculate_fid(real_imgs[:100], generated[:100])

            # EXPERIMENT 1
            # Now FID will be based on the same generated samples each time
            fixed_generated = generator.predict([FIXED_NOISE, FIXED_LABELS_ONEHOT], verbose=0)
            real_sample = train_data[:100]
            fid = calculate_fid(real_sample, fixed_generated)

            log_message(f"Epoch {epoch+1} — FID: {fid:.4f} | G Loss: {g_loss:.4f} | C Loss: {c_loss:.4f}")
            ...
            if fid < best_fid:
                ...
                critic.save_weights(os.path.join(CHECKPOINT_DIR, "best_critic.h5"))
                log_message(f"Best FID improved to {fid:.4f}. Models checkpointed.")
            else:
                wait += 1
                log_message(f"No improvement in FID for {wait} evaluations.")

            if early_stop_enabled and wait >= patience:
                log_message("Early stopping triggered. No FID improvement.")
                break

            save_samples(generator, epoch)
            generator.save_weights(os.path.join(CHECKPOINT_DIR, f"generator_epoch_{epoch}.h5"))
            critic.save_weights(os.path.join(CHECKPOINT_DIR, f"critic_epoch_{epoch}.h5"))
...
""" ===== START Sample Generation & Evaluation with Classifier ===== """

# -----------------------------------------
# SECTION 14: Generate Synthetic Samples per Class
# -----------------------------------------
def generate_samples_per_class(generator, samples_per_class=2000, output_dir=WGAN_SYNTHETIC_DIR):
    os.makedirs(output_dir, exist_ok=True)
    ...
    for class_idx in range(NUM_CLASSES):
        ...
        log_message(f"{samples_per_class} samples generated for class {class_idx} and saved.")

# -----------------------------------------
# SECTION 15: Load Synthetic Samples
# -----------------------------------------
def load_synthetic_samples(directory=WGAN_SYNTHETIC_DIR):
    gen_data, gen_labels = [], []
...
# -----------------------------------------
# SECTION 16: Build Classifier Architecture
# -----------------------------------------
...
def evaluate_classifier_with_synthetic(generator):
    ...
    classifier.fit(X_combined, y_combined, batch_size=256, epochs=200, validation_split=0.1) # EXPERIMENT 4
...
""" ===== END Sample Generation & Evaluation with Classifier ===== """
...
if __name__ == "__main__":
    if RUN_MODE == "train":
        log_message("Starting Conditional WGAN-GP Training...")
        train_wgan_gp()
