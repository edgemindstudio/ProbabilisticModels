...
def save_samples(generator, epoch):
    ...
    for i in range(NUM_CLASSES):
        ...
        axs[i].axis('off')
    plt.savefig(f"logs/generated_sample_epoch_{epoch}.png")
    plt.close()
...
# Log Gradient Stats
# -------------------------------
def log_gradient_statistics(model, inputs, targets, loss_fn):
    with tf.GradientTape() as tape:
        predictions = model(inputs, training=True)
        loss = loss_fn(targets, predictions)
    grads = tape.gradient(loss, model.trainable_weights)
    with open("logs/gradient_stats.txt", "a") as f:
        for i, grad in enumerate(grads):
            if grad is not None:
                grad_norm = tf.norm(grad).numpy()
                grad_mean = tf.reduce_mean(grad).numpy()
                grad_std = tf.math.reduce_std(grad).numpy()
                f.write(f"Layer {i}: norm={grad_norm:.6f}, mean={grad_mean:.6f}, std={grad_std:.6f}\n")


# -------------------------------
# Log Intermediate Activations
# -------------------------------
def log_activations(model, input_data, layer_names, prefix):
    intermediate_models = [models.Model(model.input, model.get_layer(name).output) for name in layer_names]
    with open(f"logs/{prefix}_activations.txt", "a") as f:
        for name, imodel in zip(layer_names, intermediate_models):
            output = imodel.predict(input_data, verbose=0)
            f.write(
                f"{name} - mean: {np.mean(output):.6f}, std: {np.std(output):.6f}, max: {np.max(output):.6f}, min: {np.min(output):.6f}\n")
...
def train_cdgan():
    ...
    d_losses, g_losses, fid_scores, epoch_times = [], [], [], []
    ...
    for epoch in trange(EPOCHS):
        ...
        # Log gradient and activation stats
        log_gradient_statistics(combined, [noise, real_lbls], misleading_y, tf.keras.losses.BinaryCrossentropy())
        log_activations(generator, [noise, real_lbls], ["gen_deconv1", "gen_deconv2"], prefix="generator")
        log_activations(discriminator, [real_imgs, real_lbls], ["disc_conv1", "disc_conv2"], prefix="discriminator")
        ...
        if epoch % 1000 == 0:
            ...
            print(f"FID Score: {fid:.2f} | Time per epoch: {epoch_times[-1]:.2f}s")
            ...
            plt.savefig("logs/loss_fid_curve.png")
            plt.close()
