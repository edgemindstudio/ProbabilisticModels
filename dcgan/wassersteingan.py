...
def train_dcgan():
    ...
    for epoch in trange(EPOCHS):
        ...
        if epoch % 1000 == 0:
            ...
            # New: Print discriminator accuracy
            real_scores = discriminator.predict(real_imgs[:100])
            fake_scores = discriminator.predict(generator.predict(np.random.normal(0, 1, (100, LATENT_DIM))))
            real_acc = np.mean(real_scores > fake_scores)
            fake_acc = np.mean(fake_scores < real_scores)
            print(f"Discriminator Accuracy → Real: {real_acc * 100:.2f}%, Fake: {fake_acc * 100:.2f}%")
